...
VALIDITY () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/${GitUser}/permission/main/ipvps.conf | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/${GitUser}/permission/main/ipvps.conf | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
export trojanlink3="trojan://${user}@www.speedtest.net:443?path=wss://www.speedtest.net$patchtls&security=tls&host=${sts}${domain}&type=ws&sni=www.speedtest.net#${user}"
