...
adminip=$( curl -sS https://raw.githubusercontent.com/MZTechTV/allow/main/adminipvps | grep $MYIP )
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/MZTechTV/permission/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
git config --global user.name "MZTechTV"
git clone https://github.com/MZTechTV/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/MZTechTV/allow.git
git push -f https://${tokengit}@github.com/MZTechTV/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/MZTechTV/multiport/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
